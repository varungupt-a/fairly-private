...
Case 1: neighboring database $Z'$ is a change in entry within a subgroup ie. within $Z_{1a}$. Let $z_0$ be replaced by $z_1$ then $\gamma^{Z'}_{1\neg a} = \gamma^Z_{1\neg a}$ but

$$\gamma_{1a}^Z(h) = \frac{1}{|Z_{1a}|} \sum_{Z_{1a}} h(x,a)$$


Case 2: neighboring database is not a change in entry within a subgroup. Thus deleting an entry in $Z_{1a}$ and add one to $Z_{1 \neg a}$.
