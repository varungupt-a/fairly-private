...
def Train(X, y, s, network_parameters, num_steps,
...
#####################################
    ...
    s = tf.placeholder(tf.int8, [None, 2]) #binary class membership
    # Set model weights
    W = tf.Variable(tf.zeros([784, 10]))
    b = tf.Variable(tf.zeros([10]))

    pred = tf.nn.softmax(tf.matmul(x, W) + b) # Softmax logistic regression predictor

    #construct fair regularizer
    
    r = tf.reduce_sum(pred) / n_samples
    ##TODO: qs
    
    fairReg = tf.reduce_sum(pred * (tf.log(qs) - tf.log(r))
                            + (1-pred)*(tf.log(1.0-qs)-tf.log(1.0-r)))
    # Minimize error and fairness loss with regularizer

    cost = tf.reduce_mean(-(y*tf.log(pred) + (1.0-y)*(tf.log(1.0-pred)),
                                         reduction_indices=1)
                          + eta * fairReg
                          + 0.5 * beta * tf.nn.l2_loss(W))

    
    # Gradient Descent set beow
